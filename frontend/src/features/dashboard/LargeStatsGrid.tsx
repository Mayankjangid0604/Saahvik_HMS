...
function methodBreakdown(byMethod?: CollectionByMethod, total: number = 0): BreakdownItem[] {
  if (!byMethod) return [];
  return (Object.keys(METHOD_LABELS) as PaymentMethod[])
    .map((m) => ({
      key: m,
      label: METHOD_LABELS[m],
      amountPaisa: byMethod[m] ?? 0,
      pct: total === 0 ? 0 : Math.round(((byMethod[m] ?? 0) / total) * 100),
    }))
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <BigCard label="Occupancy">
        <p className="mt-1 font-mono text-2xl font-semibold text-primary">{occupancy?.occupancyPct ?? 0}%</p>
        <p className="mt-0.5 text-xs text-muted">
          {occupancy?.occupiedBeds ?? 0}/{occupancy?.totalBeds ?? 0} beds occupied
        </p>
        {showProperties && (
      ...
      <BigCard label="Outstanding Dues">
        <MoneyDisplay
          paisa={dues?.totalDuesPaisa ?? 0}
          className={cn("mt-1 text-2xl font-semibold", (dues?.totalDuesPaisa ?? 0) > 0 ? "text-red-600" : "text-green-700")}
        />
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-100 pt-3 text-xs">
          <div>
            <p className="font-mono text-sm font-semibold text-ink">{dues?.residentsWithDues ?? 0}</p>
            <p className="text-muted">residents</p>
          </div>
          <div>
            <p className="font-mono text-sm font-semibold text-red-600">{dues?.highSeverityCount ?? 0}</p>
      ...
      <BigCard label="Total Collection">
        <MoneyDisplay paisa={collection?.totalPaisa ?? 0} className="mt-1 text-2xl font-semibold text-primary" />
        <div className="mt-3 border-t border-slate-100 pt-3">
          <BreakdownBars items={methodBreakdown(collection?.byMethod, collection?.totalPaisa ?? 0)} />
        </div>
      </BigCard>
